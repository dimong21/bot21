...
class VKChatManager:
    def __init__(self, group_token, group_id):
        ...
        # Только эти пользователи могут использовать /agent add/del
        self.super_admins = [
    ...
    def create_start_keyboard(self):
        return self.create_inline_keyboard([{'text': '🚀 Активировать бота', 'color': 'primary'}])
    
    def create_staff_keyboard(self):
        return self.create_inline_keyboard([{'text': '👥 Показать с никами', 'color': 'primary'}])
    ...
    def update_agent_permissions(self, admin_id, target_id, permission, value):
        ...
        if self.is_super_admin(admin_id):
            can_manage = True
        elif self.has_agent_permission(admin_id, 'agent'):
            can_manage = True
    ...
    def get_agent_info(self, admin_id, target_id):
        if not self.is_super_admin(admin_id) and not self.has_agent_permission(admin_id, 'agent'):
    ...
    def handle_message(self, event):
        if event.type == VkBotEventType.MESSAGE_NEW:
            ...
            if 'text' in message:
                ...
                # Команда /agent - только для супер-админов
                        ...
                        if len(parts) >= 3:
                            if parts[1] == 'add':
                                ...
                                try:
                                    ...
                                    success, msg = self.update_agent_permissions(user_id, target_id, permission, value)
                                    self.send_message(msg, chat_id)
                                except ValueError:
                                    self.send_message("❌ Неверный формат!", chat_id)
                                return
                            else:
                                self.send_message("❌ Неизвестная подкоманда! Доступны: add, del, info, set", chat_id)
                        else:
                            self.send_message("🔧 **Управление агентами**\n\nИспользование:\n/agent add [id]\n/agent del [id]\n/agent info [id]\n/agent set [id] [право] [true/false]", chat_id)
                    else:
                        if self.has_agent_permission(user_id, 'agent'):
                            self.send_message("🔧 **Управление доступами агента**\n\nВведите ID агента для настройки:", chat_id)
                            self.waiting_for_agent_id[user_id] = True
                        else:
                            self.send_message("❌ У вас нет прав для управления агентами!", chat_id)
                    return
                ...
                if text in self.commands['givemoney'] and self.has_agent_permission(user_id, 'givemoney'):
                    parts = text.split()
                    if len(parts) >= 4:
                        try:
                ...
                if text in self.commands['givevip'] and self.has_agent_permission(user_id, 'givevip'):
                    parts = text.split()
                    if len(parts) >= 3:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['sysban'] and self.has_agent_permission(user_id, 'sysban'):
                    parts = text.split()
                    if len(parts) >= 3:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['sysunban'] and self.has_agent_permission(user_id, 'sysban'):
                    parts = text.split()
                    if len(parts) >= 2:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['sysrole'] and self.has_agent_permission(user_id, 'sysrole'):
                    parts = text.split()
                    if len(parts) >= 3:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['sysinfo'] and self.has_agent_permission(user_id, 'sysinfo'):
                    parts = text.split()
                    if len(parts) >= 2:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['snick'] and self.has_agent_permission(user_id, 'snick'):
                    parts = text.split()
                    if len(parts) >= 3:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['rnick'] and self.has_agent_permission(user_id, 'rnick'):
                    parts = text.split()
                    if len(parts) >= 2:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['delkick'] and self.has_agent_permission(user_id, 'delkick'):
                    success, msg = self.kick_banned_accounts(user_id)
                    self.send_message(msg, chat_id)
                    return
                
                if text in self.commands['nonames'] and self.has_agent_permission(user_id, 'snick'):
                ...
                if text in self.commands['ponicku'] and self.has_agent_permission(user_id, 'snick'):
                    parts = text.split()
                    if len(parts) >= 2:
                        search = parts[1]
                        ...
                        if users:
                            ...
                            for uid, name, nickname in users[:20]:
                                msg += f"• [id{uid}|{name}] - ник: {nickname}\n"
                            if len(users) > 20:
                                msg += f"\n... и еще {len(users) - 20} пользователей"
                            self.send_message(msg, chat_id)
                        else:
                ...
                if text in self.commands['mutereports'] and self.has_agent_permission(user_id, 'mutereports'):
                    parts = text.split()
                    if len(parts) >= 2:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['unmutereports'] and self.has_agent_permission(user_id, 'unmutereports'):
                    parts = text.split()
                    if len(parts) >= 2:
                        try:
                            target_id = int(parts[1])
                ...
                if text in self.commands['vip']:
                    ...
                    keyboard = self.create_inline_keyboard([{'text': '💎 Купить VIP', 'color': 'primary'}])
    ...
    def handle_callback_query(self, event):
        if event.type == VkBotEventType.MESSAGE_EVENT:
            ...
            if 'payload' in payload:
                ...
                if data.startswith('agent_perm_') and (self.is_super_admin(user_id) or self.has_agent_permission(user_id, 'agent')):
                    parts = data.split('_')
                    if len(parts) >= 4:
                        target_id = int(parts[2])
                        permission = parts[3]
